...
public struct MTPSpeculativeTokenIterator: TokenIteratorProtocol {
    ...
    /// proposal distribution includes suppression and history penalties.
    let draftSampler: LogitSampler

    /// Compatibility sampler for older greedy-only drafters whose API cannot
    /// receive a processor. It at least masks target-suppressed token IDs.
    ...
    public init(
        ...
        if let suppressor = makeSuppressTokensProcessor(model: mainModel) {
            self.legacyDraftSampler = SuppressTokensSampler(
                base: drafterSampler, suppressor: suppressor)
        } else {
