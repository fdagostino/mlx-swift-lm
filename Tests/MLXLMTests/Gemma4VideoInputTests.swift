...
struct Gemma4VideoInputTests {
    ...
    func imageAndVideoAgree() throws {
        ...
        func lastLogits(mm: Int, asVideo: Bool) throws -> MLXArray {
            ...
            let result = try model.prepare(
                input, cache: model.newCache(parameters: nil), windowSize: 1024)
    ...
    func videoTruncatesPerFrame() throws {
        ...
        let pixels =
            (MLXArray(0 ..< (numFrames * 3 * 4 * 4)).reshaped([numFrames, 3, 4, 4])
    ...
    func processorExtractsFrames() async throws {
        let processor = Gemma4Processor(try Self.makeProcessorConfig(), tokenizer: VideoTestTokenizer())
